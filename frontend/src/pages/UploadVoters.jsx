    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            if (parsedVoters.length === 0) {
                setError('No valid voters found. Ensure your file has an "Email" column.');
                return;
            }

            setVoters(parsedVoters);
        } catch (err) {
            setError('Failed to parse file. Make sure it is a valid .csv or .xlsx format.');
        }
    };
            ...
            <div className='w-full max-w-4xl bg-white border border-gray-200 px-6 sm:px-10 py-8'>
                ...
                <div className='mb-8'>
                    ...
                    <p className='text-[16px] text-gray-500'>
                        Step 2: Submit a CSV or Excel (.xlsx) file containing an <span className="font-medium text-gray-700">Email</span> column. Only these users will be allowed to vote in your election.
                    </p>
                </div>
                ...
                <div className='flex flex-col gap-4 mb-8'>
                    <label className='flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition'>
                        <div className='flex flex-col items-center justify-center pt-5 pb-6'>
                            ...
                            <p className="mb-2 text-sm text-gray-500 font-semibold">{fileName ? fileName : 'Click to upload CSV or Excel'}</p>
                ...
                {voters.length > 0 && (
                    <div className='mb-8 border border-gray-200 rounded overflow-hidden'>
                        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                            <h3 className="font-semibold text-gray-700 inter-font">Import Preview</h3>
                            <span className="text-sm text-[#00263A] font-bold bg-[#00263A]/10 px-3 py-1 rounded-full">{voters.length} Valid Voters</span>
                        </div>
                        <div className="max-h-64 overflow-y-auto">
                            <table className="min-w-full divide-y divide-gray-200 text-sm text-left">
                                <thead className="bg-[#FAFAFA] sticky top-0">
                                    <tr>
                                        <th scope="col" className="px-6 py-3 font-semibold text-gray-600">Email Address</th>
                                    </tr>
                                </thead>
                                    ...
                                    {voters.length > 100 && (
                                        <tr>
                                            <td className="px-6 py-4 text-center text-gray-500 italic bg-gray-50">
                                                ... and {voters.length - 100} more rows (not shown for performance)
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
