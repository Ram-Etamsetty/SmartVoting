...
const StatCard = ({ title, value, subtitle, icon }) => (
    <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        ...
        <div className="flex flex-col">
            <span className="text-3xl font-bold text-[#00263A] inter-font">{value}</span>
...
const ElectionDetails = () => {
    ...
    const votedCount = election.allowedVoters?.filter(v => v.hasVoted).length || 0;
    const pendingCount = totalVoters - votedCount;
    const turnoutPercentage = totalVoters === 0 ? 0 : Math.round((votedCount / totalVoters) * 100);

    // Color palette for charts
    const COLORS = ['#00263A', '#F28A36', '#3b82f6', '#10b981', '#f43f5e', '#8b5cf6', '#f59e0b'];
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#FAFAFA]">
            ...
            <main className="flex-1 px-4 sm:px-8 py-8 w-full max-w-7xl mx-auto">
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                        <div>
                            <div className="flex items-center gap-4 mb-2">
                                <h1 className="text-2xl sm:text-3xl font-bold text-[#00263A] inter-font">{election.title}</h1>
                                <StatusBadge status={election.status} />
                            </div>
                            <p className="text-gray-500 max-w-3xl leading-relaxed">{election.description || "No description provided."}</p>
                            
                            <div className="flex flex-wrap items-center gap-x-8 gap-y-3 mt-6">
                                <div className="flex flex-col">
                                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Start Date</span>
                                    <span className="text-sm text-[#262D3A] font-medium">{formatDate(election.startDate)}</span>
                                </div>
                                <div className="flex flex-col">
                                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">End Date</span>
                                    <span className="text-sm text-[#262D3A] font-medium">{formatDate(election.endDate)}</span>
                                </div>
                                <div className="flex flex-col">
                                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Election Type</span>
                                    <span className="text-sm text-[#262D3A] font-medium capitalize">{election.type}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {/* Voter Statistics */}
                <h2 className="text-xl font-bold text-[#262D3A] mb-4 inter-font">Voter Turnout & Participation</h2>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 flex flex-col mb-10">
                    <div className="flex sm:items-center justify-between flex-col sm:flex-row gap-4 mb-6">
                        <div>
                            <h2 className="text-xl font-bold text-[#262D3A] inter-font">Election Results Data</h2>
                            <p className="text-sm text-gray-500 mt-1">Official tallying algorithms</p>
                        </div>
                        ...
                        {election.status === 'closed' ? (
                            ...
                            <div className="px-5 py-2.5 bg-gray-50 border border-gray-200 rounded-lg flex items-center gap-2">
                                <span>🔒</span>
                                <span className="text-sm font-medium text-gray-500 tracking-wide">RESULTS BOUND (ACTIVE)</span>
                            </div>
                        )}
                    </div>
                    ...
                    {!showResults ? (
                        <div className="w-full py-16 flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-lg bg-gray-50">
                            <span className="text-4xl mb-3">{election.status === 'closed' ? "📊" : "🔒"}</span>
                            <h3 className="text-[#262D3A] font-bold text-lg mb-1">
                                {election.status === 'closed' ? "Data Compiled & Ready" : "Election In Progress"}
                            </h3>
                        ...
                        <div className="flex flex-col lg:flex-row gap-10">
                            ...
                            <div className="w-full lg:w-2/5">
                                <h3 className="text-lg font-bold text-[#00263A] mb-4">Official Tally Ledger</h3>
                                <div className="flex flex-col gap-3">
                                    {[...election.candidates].sort((a,b) => b.voteCount - a.voteCount).map((candidate, idx, arr) => {
                                        const maxVotes = arr[0].voteCount;
                                        const isTied = maxVotes > 0 && arr.filter(c => c.voteCount === maxVotes).length > 1;
                                        ...
                                        <div key={idx} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg bg-gray-50 hover:bg-gray-100 transition">
                                            <div className="flex flex-col">
                                                <span className="font-semibold text-[#262D3A]">{candidate.name}</span>
                                                {candidate.party && <span className="text-xs text-gray-400 uppercase tracking-widest mt-1">{candidate.party}</span>}
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <div className="text-right">
                                                    <span className="block text-xl font-bold text-[#00263A] leading-none">{candidate.voteCount}</span>
                                                    <span className="text-xs text-gray-500">Votes</span>
                                                </div>
                                                {isWinner && (
                                                    <span className="text-xl" title={isTied ? "Tied for Lead" : "Current Leader"}>{isTied ? "🤝" : "👑"}</span>
                                                )}
                                            </div>
                                        </div>
                                    )})}
                    ...
                    {showLogs && (
                        <div className="mt-10 border-t border-gray-100 pt-8">
                            <div className="flex justify-between items-center mb-4">
                                <div>
                                    <h3 className="text-lg font-bold text-[#00263A] inter-font">Decoupled Audit Ledger</h3>
                                    <p className="text-sm text-gray-500">Immutable cryptographic log of all securely recorded ballots</p>
                                </div>
                                <span className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-bold">{ballots.length} Records</span>
                            </div>
                            
                            <div className="bg-gray-50 rounded-lg max-h-96 overflow-y-auto border border-gray-200">
                                {ballots.length === 0 ? (
                                    <p className="text-center text-gray-400 py-8">No immutable audit logs found for this election.</p>
                                ) : (
                                    <table className="w-full text-sm text-left">
                                        ...
                                        <tbody>
                                            {ballots.map(b => (
                                                <tr key={b._id} className="border-b border-gray-200 hover:bg-white transition">
                                                    <td className="px-6 py-4 font-medium text-[#262D3A]">
                                                        {new Date(b.timestamp).toISOString().replace('T', ' ').slice(0, 19)}
                                                    </td>
